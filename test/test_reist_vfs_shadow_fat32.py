...
class ReistVfsShadowFat32Tests(unittest.TestCase):
    ...
    def test_parser_has_fixed_authority_and_work_bounds(self):
        ...
        self.assertIn("shadow_file_chain_guard_t", source)
        self.assertIn("shadow_file_chain_advance", source)
        self.assertNotIn("visited[REIST_VFS_SHADOW_MAX_FILE_CHAIN_CLUSTERS]",
                         source)
        ...
        self.assertIn("reist_vfs_shadow_fat_readdir_cursor_t", header)
        self.assertIn("reist_vfs_shadow_fat_readdir_continue", header)
        self.assertIn("shadow_readdir_resume_valid", source)
        self.assertIn("shadow_readdir_chain_position", source)
