...
class PartitionContractTests(unittest.TestCase):
    ...
    def test_mbr_children_are_bounded_and_fail_closed(self):
        ...
        self.assertIn("discover_gpt", source)
        self.assertIn("GPT_MAX_ENTRIES 128U", source)
        self.assertIn("entries_crc ^ 0xFFFFFFFFU", source)
        self.assertIn("header_crc", source)
        self.assertIn("candidates_valid", source)
        self.assertIn("(uint64_t)first + sectors > parent->sectors", source)
        self.assertIn("(size_t)drive_count + count > MAX_DRIVES", source)
        ...
        self.assertIn("partition_discover();", kernel)
