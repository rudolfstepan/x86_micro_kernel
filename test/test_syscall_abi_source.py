...
class SyscallAbiSourceTests(unittest.TestCase):
    ...
    def test_common_list_is_complete_append_only_v1(self) -> None:
        ...
        self.assertIn("REIST_SYSCALL_COUNT 130U", self.common)
        entries = re.findall(
            r"^\s*X\(([A-Z][A-Z0-9_]*), ([A-Z][A-Z0-9_]*), "
            r"([0-9]+)U\)", self.common, re.MULTILINE)
        self.assertEqual(len(entries), 130)
        self.assertEqual([int(item[2]) for item in entries], list(range(130)))
        self.assertEqual(len({item[0] for item in entries}), 130)
        self.assertEqual(len({item[1] for item in entries}), 130)
        ...
        self.assertIn("PROCESS_DOMAIN_SYSCALL_LIMIT 130U", self.process)
    ...
    def test_check_mode_is_read_only_and_in_sync(self) -> None:
        ...
        self.assertIn("SYSCALL_ABI_CHECK_OK count=130", result.stdout)
