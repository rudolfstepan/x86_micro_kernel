"""Broad, bounded GNU Unifont BMP fallback contracts."""
...
SOURCE = ROOT / "assets/fonts/source/unifont-16.0.04.hex.gz"
FONT = ROOT / "assets/fonts/reist-unicode-bmp.psf"
...
class GuiBmpFontTests(unittest.TestCase):
    ...
    def test_psf2_has_exact_bounded_source_coverage(self) -> None:
        ...
        self.assertEqual(header, (0x864AB572, 0, 32, 1,
                                  57086, 16, 16, 8))
        self.assertEqual(len(glyphs), 57086)
        self.assertEqual(len({scalar for scalar, _ in glyphs}), 57086)
        self.assertLess(len(data), 2 * 1024 * 1024)
        table = data[32 + 57086 * 16:]
        expected = b"".join(
    ...
    def test_desktop_uses_fixed_bmp_fallback_after_cp437(self) -> None:
        ...
        self.assertNotIn("x86os_read(descriptor, bytes + used, capacity - used)",
                         bounded_read)
        self.assertIn("DESKTOP_FONT_MAPPING_CAPACITY 65536U", desktop)
        self.assertIn("/usr/share/fonts/reist-unicode-bmp.psf", desktop)
        ...
        self.assertIn("font_overlay_status != 7", desktop)
    ...
    def test_full_images_include_assets_and_floppy_excludes_them(self) -> None:
        ...
        windows = (ROOT / "scripts/build-windows.ps1").read_text(
            encoding="utf-8")
        for path in ("usr/share/fonts/reist-unicode-bmp.psf",
        ...
        self.assertIn("usr/share/fonts/reist-unicode-bmp.psf=%",
                      filter_section)
        self.assertNotIn("$floppyDataArguments += @(\n        '--data-file', "
                         "\"usr/share/fonts/reist-unicode-bmp.psf", windows)
