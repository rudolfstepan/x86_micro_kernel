...
static bool header_valid(const fat12_remap_header_t *header,
    ...
    copy.crc32 = 0U;
    return crc == fat12_remap_crc32(&copy, sizeof(copy));
}
...
static void prepare_header(fat12_remap_header_t *header, uint32_t fingerprint,
    ...
    *header = (fat12_remap_header_t){
    ...
    header->crc32 = fat12_remap_crc32(header, sizeof(*header));
}
...
bool fat12_remap_load(fat12_remap_table_t *table, fat12_remap_read_fn read,
    ...
    if (table == NULL || read == NULL || table->media_fingerprint == 0U)
        return false;
    uint8_t primary[512], mirror[512];
...
static bool write_headers(const fat12_remap_table_t *table,
    ...
    if (!write(context, table->primary_sector, sector)) return false;
    return write(context, table->mirror_sector, sector);
}

bool fat12_remap_add(fat12_remap_table_t *table, uint32_t bad_sector,
        uint32_t replacement_sector, fat12_remap_write_fn write,
        void *context) {
    if (table == NULL || write == NULL || bad_sector == 0U ||
    ...
    if (!write(context, table->table_sector, sector)) return false;
    ...
    return write_headers(table, write, context);
}
