...
int JsSession::script(const void *data,uint32_t size,void *out,uint32_t cap,uint32_t budget) {
    if(!ready() || header_.operation!=JS_OP_HELLO || !data || size<24 ||
       size>JS_SERVICE_SCRIPT_SOURCE || !out || cap<24 || cap>JS_SERVICE_RESULT) return -22;
